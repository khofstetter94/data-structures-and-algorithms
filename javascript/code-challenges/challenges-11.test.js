...
const totalSum = (input) => {
  // Solution code here...
  return input.flat().reduce((preVal, current) => preVal + current, 0);
};
...
const divisibleByFiveTwoToThePower = (input) => {
  ...
  for(let i = 0; i < input.length; i++){
    let filtered = input[i].filter(element => typeof element === 'number' && element % 5 === 0);
    let halfwayArr = [];
    for(let j = 0; j < filtered.length; j++){
      halfwayArr.push(Math.pow(2, filtered[j]));
    }
    finalArr.push(halfwayArr);
  }
  return finalArr;
};
...
let findMaleAndFemale = (data) => {
  ...
  data.forEach(obj => {
    if (obj.gender === 'male' || obj.gender === 'female'){
      genderName.push(obj.name);
    }
  });
  let final = genderName.join(' and ');
  return final;
};
...
let findShortest = (data) => {
  ...
  let name = null;
  data.map(each => {
    let height = parseInt(each.height);
    if (height < shortest) {
      shortest = each.height;
      name = each.name;
    }
  });
  return name;
