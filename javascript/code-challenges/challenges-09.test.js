...
const updateNumbers = (obj) => {
  ...
  let finalArr = entries.map(a => {
    let combined = `${a[0]}: ${a[1]}`;
    return combined;
  });
  return finalArr;
};
...
const hasChildrenValues = (arr, character) => {
  // Solution code here...
  // Find objects in the array that have the key children, check for length of array of children
  let foundCharacter = Object.values(arr).find(obj => {
    return (
